...
import ca.uqac.lif.cep.methods.MethodEvent.MethodKey;
 ...
 */
public class isKey extends UnaryFunction<MethodEvent,Integer> 
{
	/**
	...
	public Integer getValue(MethodEvent x) throws FunctionException 
	{
		...
		MethodKey call = (MethodKey) x;
		
	//	System.out.println(call.toString()+" nombre "+call.KeyIndex+" parametre "+call.m_arguments.length);
		...
		try {
		res=ChecKey((String)call.getKey(),call.m_methodName);
		} catch (IOException e) {
	...
	protected static int ChecKey(String Key,String MethodName) throws IOException
	{	
	// TODO Auto-generated method stub
		
	 
		File fil =new File("keys/"+MethodName+"Keys.txt");
		if (!fil.exists()){fil.createNewFile();}
	Scanner input =  new Scanner(fil);
	//System.out.println(input.nextLine());
	Boolean found = false;
	//System.out.println("Calling "+MethodName+" With key : "+Key );
