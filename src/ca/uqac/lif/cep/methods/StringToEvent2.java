	...
	public MethodEvent2 getValue(String x)
	{
		...
		{	
			...
			if (!parts[3].equals("NAN")){
				...
				for (int i = 3+types.length,j=0,k=2; i < parts.length-k; i++,j++,k++)
				{			
					args[j] = parts[i];
			...
			return new MethodCall2( parts[2], parts[1], null, parts[parts.length-2], parts[parts.length-1],types ,args);
		}
		else 
		{
			if (parts[0].compareToIgnoreCase("output") == 0)
				...
				return new MethodKey2(parts[2], parts[1], null, parts[parts.length-2], parts[parts.length-2],ind, types,args);
				}
			}
	}
}
