...
public class StringToEvent extends UnaryFunction<String,MethodEvent>
{
	...
	public MethodEvent getValue(String x)
	{
		...
		{
			if (parts[0].compareToIgnoreCase("return") == 0)
				return new MethodReturn(parts[1].trim());
			else{
			String[] parts2 = x.split("//");
			
			int ind = Integer.parseInt(parts2[1].trim());
			
			String[] parts3 = parts2[0].split("\\s+");
			
			Object[] args = new Object[parts3.length - 2];
			...
			return new MethodKey(parts3[1],ind,args);}}
	}
}
