...
public class SafeHashMap {
	...
	public static void main(String[] args) throws ConnectorException, URISyntaxException, IOException {
		...
		String filename = "traces/SafeHachMapTrace.txt";
		URI uri = SafeEnum.class.getResource("objects.txt").toURI();
		String pathObj = Paths.get(uri).toString();FunctionProcessor converter = new FunctionProcessor(StringToEvent2.instance);
		...
		Function check_anno = new CheckAnnotation(new Scanner(SafeEnum.class.getResourceAsStream("MethodsSignatures/SafeHachMapSignature2.txt")));
		Function check_annox = new CheckAnnotation(new Scanner(SafeEnum.class.getResourceAsStream("MethodsSignatures/SafeHachMapSignature.txt")));
		Function checknF = new cheknfileHM(pathObj);
